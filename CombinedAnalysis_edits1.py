...
for x in range(1,len(dataframes)+1):
    ...
    for z in range(1, len(LBMPvals)):
        ...
        if ((intIndex == chargeIndex[r]) & (SOE_array[y-1]<dischargeEnergyCap)):
            ...
            Hourly_Charging_Cost[y] = -1* LBMPvals[y] * chargeTime * 0.001
        ...
        #charge the battery if the time if LBMP high, the battery has charge, and we have not yet reached our max daily discharging throughput
            ...
            if (intIndex == dischargeRef):
                ...
                val = SOE_array[y-1] * RTefficiency #take into account roundtrip efficiency (get less output power than input)
                dischargeEnergy = val
                loss = SOE_array[y-1] * (1-RTefficiency) #losses that we experience when discharging battery

            SOE_array[y] = SOE_array[y - 1] - dischargeEnergy - loss
            Battery_Output_array[y] = dischargeEnergy
            Hourly_Revenue_array[y] = LBMPvals[y] * dischargeTime *0.001
...
#plot our outputs
plt.plot(hourly_BatteryDischarge)
plt.plot(hourlyLBMP)
plt.legend()
...
##########################################
#monthsList = []
#for g, df in df_Year.groupby(np.arange(len(df_Year['Hourly_Revenue_Generation']))//(week*hours*30)):
    #monthsList.append(df)
